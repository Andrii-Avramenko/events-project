...
export function renderModal(event) {
  ...
  const mainEventImg = event.images.find(
    ({ ratio, width }) => ratio === '16_9' && width === 1024
  );
  ...
  const markup = `<img src="${previewEventImg.url}" alt="" class="modal-artist-preview" />
      <div class="modal-main">
        <img src="${mainEventImg.url}" alt="" class="modal-artist-main" />
        <div class="modal-info">
          <section>
            <h2 class="modal-title">Info</h2>
            <p class="modal-desc">${event.info ? event.info.slice(0, 150) + '…' : 'No information provided'}
            </p>
          </section>
          <section>
            ...
            <p class="modal-desc">
              ${event.dates.start.localTime.slice(0, 5)} (${event.dates.timezone})
            </p>
          </section>
          <section>
            <h2 class="modal-title">Where</h2>
            <p class="modal-desc">${event._embedded.venues[0].city.name}, ${event._embedded.venues[0].country.name}</p>
          ...
          <section>
            ...
            <a href="${event.url}" target="_blank"
              ><button type="button" class="modal-tickets">
                View Tickets
              </button></a>
          </section>
        </div>
      </div>
      <a href="${event._embedded.attractions[0].url}" target="_blank" class="modal-more" ><button type="button">MORE FROM THIS AUTHOR</button></a>`;
